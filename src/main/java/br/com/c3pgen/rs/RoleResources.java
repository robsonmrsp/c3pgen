...
import br.com.c3pgen.service.RoleService;
...
public class RoleResources {
	...
	public Response save(JsonRole jsonRole) {
		try {
			...
			return Response.serverError().entity(new JsonError(message, jsonRole, e.getLegalMessage())).build();
		} catch (Exception e) {
			String message = String.format("Não foi possivel salvar  role [ %s ] parametros [ %s ]", e.getMessage(), jsonRole.toString());
			LOGGER.error(message, e);
			return Response.serverError().entity(new JsonError(message, jsonRole)).build();
		}
	}
