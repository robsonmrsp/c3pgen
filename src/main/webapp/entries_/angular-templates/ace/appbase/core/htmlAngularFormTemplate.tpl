						...
						<div class="panel panel-default plain profile-widget">
							...
							<#list entity.attributes as att>
							  <#if att.viewApproach?? >
								...
								<div id="groupInput${firstUpper(att.name)}" class="form-group">
									<label class="control-label" for="input${firstUpper(att.name)}">${firstUpper(att.displayName)!firstLower(att.name)} ${att.required ? string('*','')} </label>
									<select class="form-control" data-ng-model="${firstLower(entity.name)}Model.${firstLower(att.name)}" ng-options="${firstLower(att.name)} as ${firstLower(att.name)}.${firstLower(rel.viewApproach.textField)} for ${firstLower(att.name)} in ${firstLower(att.name)}s"></select>
								</div>					
							...
							<#list entity.relationships as rel>
								...
								<div id="groupInput${firstUpper(rel.name)}" class="form-group">
									...
									<select class="form-control" id="input${firstUpper(rel.name)}"  data-ng-model="${firstLower(entity.name)}Model.${firstLower(rel.name)}" ng-options="${firstLower(rel.name)} as ${firstLower(rel.name)}.${firstLower(rel.viewApproach.comboVal)} for ${firstLower(rel.name)} in ${firstLower(rel.name)}s"></select>
								</div>					
								...
								<div id="groupInput${firstUpper(rel.name)}Container" class="form-group">
									...
									<div id="groupInput${firstUpper(rel.name)}" class="input-group">
										...
										<input id="input${firstUpper(rel.name)}${firstUpper(rel.viewApproach.hiddenField)}" type="text" class="form-control append-left" data-ng-model="${firstLower(entity.name)}Model.${firstLower(rel.name)}.${firstLower(rel.viewApproach.hiddenField)}" style="display: none;" />
										</#if>
										<#if rel.viewApproach.textField??>							
										<input id="input${firstUpper(rel.name)}${firstUpper(rel.viewApproach.textField)}" placeholder="Escolha um ${firstUpper(rel.displayName)}"  type="text" class="form-control append-left" data-ng-model="${firstLower(entity.name)}Model.${firstLower(rel.name)}.${firstLower(rel.viewApproach.textField)}" />
										</#if>
