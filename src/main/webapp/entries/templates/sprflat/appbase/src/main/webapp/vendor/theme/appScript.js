...
$(document).ready(function() {
	...
	$('body').sprFlat({
		...
		breadcrumbs : {
			auto : true
		// auto populate breadcrumbs via js if is false you need to provide own
		// markup see for example.
		},
		sidebar : {
			fixed : true,// fixed sidebar
			rememberToggle : true, // remember if sidebar is hided
			offCanvas : false, // make sidebar offcanvas in tablet and small
								// screens
			pinnedSearch : false
		// make search to be pinned into top
		},
		sideNav : {
			flyout : false, // make flyout menu ,please put sidebar fixed to
							// false in order to work.
			hover : false, // shows subs on hover or click
			showNotificationNumbers : 'onhover',// show how many elements menu
			...
			showSubPlus : {
				showPlus : true,// Show plus icon instead of arrow to indicate
								// sub
				sideNavPlusIcon : 'en-plus3 s16',// icon for expand state of
													// submenu
				sideNavMinusIcon : 'en-minus3 s16'// icon for closed state of
													// submenu
			},
			...
			autoCurrentSystem : {
				...
				subDir : '' // if you put template in sub dir you need to fill
							// here. example '/html'
			}
		},
		tile : {
			countNumbers : true
		// count numbers from 0 to specified value (required count plugin)
		},
		panels : {
