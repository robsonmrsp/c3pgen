...
(function() {
	...
	require.register("browser/diff.js", function(module, exports, require) {
		 ...
		 * 
		 * This library supports the following APIS: JsDiff.diffChars: Character by character diff JsDiff.diffWords: Word (as defined by \b regex) diff which ignores whitespace JsDiff.diffLines: Line based diff
		 * 
		 * JsDiff.diffCss: Diff targeted at CSS content
		 * 
		 * These methods are based on the implementation proposed in "An O(ND) Difference Algorithm and its Variations" (Myers, 1986). http://citeseerx.ist.psu.edu/viewdoc/summary?doi=10.1.1.4.6927
		 */
		var JsDiff = (function() {
			/* jshint maxparams: 5 */
	...
	require.register("interfaces/bdd.js", function(module, exports, require) {
		 ...
		 * BDD-style interface:
		 * 
		 * describe('Array', function(){ describe('#indexOf()', function(){ it('should return -1 when not present', function(){
		 * 
		 * });
		 * 
	...
	require.register("interfaces/exports.js", function(module, exports, require) {
		 ...
		 * TDD-style interface:
		 * 
		 * exports.Array = { '#indexOf()': { 'should return -1 when the value is not present': function(){
		 *  },
		 * 
	...
	require.register("interfaces/tdd.js", function(module, exports, require) {
		 ...
		 * TDD-style interface:
		 * 
		 * suite('Array', function(){ suite('#indexOf()', function(){ suiteSetup(function(){
		 * 
		 * });
		 * 
	...
	require.register("mocha.js", function(module, exports, require) {
		 ...
		 * 
		 * Options:
		 *  - `ui` name "bdd", "tdd", "exports" etc - `reporter` reporter instance, defaults to `mocha.reporters.spec` - `globals` array of accepted globals - `timeout` timeout in milliseconds - `bail` bail on the first test failure - `slow` milliseconds to wait before
		 * considering a test slow - `ignoreLeaks` ignore global leaks - `grep` string or regexp to filter tests with
		 * 
		 * @param {Object}
		 *            options
		 * @api public
	...
	require
			.register(
					...
					function(module, exports, require) {
						...
						var statsTemplate = '<ul id="mocha-stats">' + '<li class="progress"><canvas width="40" height="40"></canvas></li>' + '<li class="passes"><a href="#">passes:</a> <em>0</em></li>' + '<li class="failures"><a href="#">failures:</a> <em>0</em></li>'
						...
						function HTML(runner) {
							...
							var self = this, stats = this.stats, total = runner.total, stat = fragment(statsTemplate), items = stat.getElementsByTagName('li'), passes = items[1].getElementsByTagName('em')[0], passesLink = items[1].getElementsByTagName('a')[0], failures = items[2]
									.getElementsByTagName('em')[0], failuresLink = items[2].getElementsByTagName('a')[0], duration = items[3].getElementsByTagName('em')[0], canvas = stat.getElementsByTagName('canvas')[0], report = fragment('<ul id="mocha-report"></ul>'), stack = [ report ], progress, ctx, root = document
			...
			.register("reporters/nyan.js",
					function(module, exports, require) {
						...
						function NyanCat(runner) {
							Base.call(this, runner);
							var self = this, stats = this.stats, width = Base.window.width * .75 | 0, rainbowColors = this.rainbowColors = self.generateColors(), colorIndex = this.colorIndex = 0, numerOfLines = this.numberOfLines = 4, trajectories = this.trajectories = [ [], [],
									[], [] ], nyanCatWidth = this.nyanCatWidth = 11, trajectoryWidthMax = this.trajectoryWidthMax = (width - nyanCatWidth), scoreboardWidth = this.scoreboardWidth = 5, tick = this.tick = 0, n = 0;
	...
	require.register("runner.js", function(module, exports, require) {
		 ...
		 * 
		 * Events:
		 *  - `start` execution started - `end` execution complete - `suite` (suite) test suite execution started - `suite end` (suite) all tests (and sub-suites) have finished - `test` (test) test execution started - `test end` (test) test completed - `hook` (hook) hook
		 * execution started - `hook end` (hook) hook complete - `pass` (test) test passed - `fail` (test, err) test failed - `pending` (test) test pending
		 * 
		 * @api public
		 */
		 ...
		 * 
		 * Hook failures work in the following pattern: - If bail, then exit - Failed `before` hook skips all tests in a suite and subsuites, but jumps to corresponding `after` hook - Failed `before each` hook skips remaining tests in a suite and jumps to corresponding `after
		 * each` hook, which is run only once - Failed `after` hook does not alter execution order - Failed `after each` hook skips remaining tests in a suite and subsuites, but executes other `after each` hooks
		 * 
		 * @param {Hook}
		 *            hook
		 * @param {Error}
	...
	require.register("utils.js", function(module, exports, require) {
		...
		function highlight(js) {
			return js.replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\/\/(.*)/gm, '<span class="comment">//$1</span>').replace(/('.*?')/gm, '<span class="string">$1</span>').replace(/(\d+\.\d+)/gm, '<span class="number">$1</span>').replace(/(\d+)/gm,
					'<span class="number">$1</span>').replace(/\bnew[ \t]+(\w+)/gm, '<span class="keyword">new</span> <span class="init">$1</span>').replace(/\b(function|new|throw|return|var|if|else)\b/gm, '<span class="keyword">$1</span>')
		}
	 ...
	 * Node shims.
	 * 
	 * These are meant only to allow mocha.js to run untouched, not to allow running node code in the browser.
	 */
