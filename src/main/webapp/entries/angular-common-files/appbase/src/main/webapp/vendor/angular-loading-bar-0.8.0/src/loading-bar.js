...
(function() {
	...
	angular.module('cfp.loadingBarInterceptor', [ 'cfp.loadingBar' ]).config([ '$httpProvider', function($httpProvider) {
		...
		var interceptor = [ '$q', '$cacheFactory', '$timeout', '$rootScope', '$log', 'cfpLoadingBar', function($q, $cacheFactory, $timeout, $rootScope, $log, cfpLoadingBar) {
			...
			/**
			 * calls cfpLoadingBar.complete() which removes the loading bar from
			 * the DOM.
			 */
			 ...
			 * 
			 * @param {Object}
			 *            config the config option from the request
			...
			function isCached(config) {
				...
				// Choose the proper cache source. Borrowed from angular: $http
				// service
			...
			return {
				'request' : function(config) {
					// Check to make sure this request hasn't already been
					// cached and that
					// the requester didn't explicitly ask us to ignore this
					// request:
	...
	angular.module('cfp.loadingBar', []).provider('cfpLoadingBar', function() {
		...
		this.$get = [ '$injector', '$document', '$timeout', '$rootScope', function($injector, $document, $timeout, $rootScope) {
			...
			function _set(n) {
				...
				// increment loadingbar to give the illusion that there is
				// always
				// progress but make sure to cancel the previous timeouts so we
				// don't
			...
			/**
			 * Increments the loading bar by a random amount but slows down as
			 * it progresses
			 */
			function _inc() {
				if (_status() >= 1) {
