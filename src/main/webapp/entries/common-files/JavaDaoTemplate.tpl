...
public class Dao${entity.name} extends AccessibleHibernateDao<Filter${entity.name}, ${entity.name}> {
	...
	@Override
	protected Criteria prepareEqCriteria(Filter${entity.name} filter${entity.name}) {
	...
	<#list entity.attributes as att>
		...
		if (filter${entity.name}.get${firstUpper(att.name)}() != null) {
			criteria.add(Restrictions.eq("${att.name}", filter${entity.name}.get${firstUpper(att.name)}()));
		}
		<#else>
		if (filter${entity.name}.get${firstUpper(att.name)}() != null) {
			criteria.add(Restrictions.eq("${att.name}", filter${entity.name}.get${firstUpper(att.name)}()));
	...
	<#list entity.relationships as rel>
		...
		if (filter${entity.name}.get${firstUpper(rel.name)!firstLower(rel.model)}() != null) {
			criteria.createAlias("${firstLower(rel.name)!firstLower(rel.model)}", "${firstLower(rel.name)!firstLower(rel.model)}_");
			criteria.add(Restrictions.eq("${firstLower(rel.name)!firstLower(rel.model)}_.id", filter${entity.name}.get${firstUpper(rel.name)!firstLower(rel.model)}()));
		}
		</#if>	
	</#list>
	</#if>	
	...
	@Override
	protected Criteria prepareIlikeCriteria(Filter${entity.name} filter${entity.name}) {
		Criteria criteria = criteria();
	...
	<#list entity.attributes as att>
      	<#if att.type.className == 'String'>	
		if (filter${entity.name}.get${firstUpper(att.name)}() != null) {
			criteria.add(Restrictions.ilike("${att.name}", filter${entity.name}.get${firstUpper(att.name)}(), MatchMode.ANYWHERE));
		}
		<#else>
		if (filter${entity.name}.get${firstUpper(att.name)}() != null) {
			criteria.add(Restrictions.eq("${att.name}", filter${entity.name}.get${firstUpper(att.name)}()));
