...
(function(root, $, _, Backbone) {
	...
	/**
	 * A floating point number formatter. Doesn't understand notation at the
	 * moment.
	 * 
	...
	var Cell = Backgrid.Cell = Backbone.View.extend({
		...
		/**
		 * @property {Backgrid.CellFormatter|Object|string} [formatter=new
		 *           CellFormatter()]
		 */
	...
	var SelectCellEditor = Backgrid.SelectCellEditor = CellEditor.extend({
		...
		// template: _.template('<option value="<%- value %>" <%= selected ?
		 ...
		 * Triggers a `backgrid:edited` Backbone event from the model.
		 */
		save : function(e) {
			var model = this.model;
			var column = this.column;
	...
	var Column = Backgrid.Column = Backbone.Model.extend({
		 ...
		 * @param {string}
		 *            attrs.name The name of the model attribute.
		 * @param {string|Backgrid.Cell}
		 *            attrs.cell The cell type. If this is a string, the
		 *            capitalized form will be used to look up a cell class in
		 *            Backbone, i.e.: string => StringCell. If a Cell subclass
		 *            is supplied, it is initialized with a hash of parameters.
		 *            If a Cell instance is supplied, it is used directly.
		 * @param {string|Backgrid.HeaderCell}
		 *            [attrs.headerCell] The header cell type.
		 * @param {string}
		 *            [attrs.label] The label to show in the header.
		 * @param {boolean}
		 *            [attrs.sortable=true]
		 * @param {boolean}
		 *            [attrs.editable=true]
		 ...
		 * @param {Backgrid.CellFormatter|Object|string}
		 *            [attrs.formatter] The formatter to use to convert between
		 *            raw model values and user input.
		 * 
		 * @throws {TypeError}
		 *             If attrs.cell or attrs.options are not supplied.
		 * @throws {ReferenceError}
		 *             If attrs.cell is a string but a cell class of said name
		 *             cannot be found in the Backgrid module.
		 * 
		 * See: - Backgrid.Cell - Backgrid.CellFormatter
		 */
	...
	var HeaderCell = Backgrid.HeaderCell = Backbone.View.extend({
		...
		/**
		 * @property {null|"ascending"|"descending"} _direction The current
		 *           sorting direction of this column.
		 */
		_direction : null,
		 ...
		 * parameters, returns the current direction of this cell, otherwise
		 * sets it. If a `null` is given, sets this cell back to the default
		 * order.
		 * 
		...
		direction : function(dir) {
			if (arguments.length) {
				var a = this.$el.find('#' + this.column.get("name"));
				if (this._direction) {
				...
				if (dir && this.column.get('sortable')) {
					if (dir == 'ascending') {
						...
						// a.removeClass('icon-sort-by-alphabet-alt')
						// a.addClass('icon-sort-by-alphabet')
					} else {
						a.removeClass('fa fa-sort-alpha-asc');
						a.addClass('fa fa-sort-alpha-desc');
						// a.removeClass('icon-sort-by-alphabet')
						// a.addClass('icon-sort-by-alphabet-alt')
					}
		...
		/**
		 * Event handler for the Backbone `backgrid:sort` event. Resets this
		 * cell's direction to default if sorting is being done on another
		 * column.
		 * 
		 * @private
		 */
		 ...
		 * done with the column name, direction, comparator and a reference to
		 * the collection.
		 * 
		 * @param {string}
		 *            columnName
	...
	var Body = Backgrid.Body = Backbone.View.extend({
		 ...
		 * @param {string}
		 *            [options.emptyText] The text to display in the empty row.
		 * 
		 * @throws {TypeError}
		 *             If options.columns or options.collection is undefined.
		 * 
		...
		removeRow : function(model, collection, options) {
			...
			if (_.isUndefined(options.render) || options.render) {
				if (options.index && options.index >= 0) // added by ##Robson
	...
	var Footer = Backgrid.Footer = Backbone.View.extend({
		 ...
		 *            options
		 * @param {*}
		 *            options.parent The parent view class of this footer.
		 * @param {Backbone.Collection.
		 *            <Backgrid.Column>|Array.<Backgrid.Column>|Array.<Object>}
		 *            options.columns Column metadata.
		 * @param {Backbone.Collection}
	...
	var Grid = Backgrid.Grid = Backbone.View.extend({
		 ...
		 * @param {boolean}
		 *            [options.render=true] Whether to render the column
