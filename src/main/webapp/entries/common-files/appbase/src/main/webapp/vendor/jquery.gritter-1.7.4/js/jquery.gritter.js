...
(function($) {
	...
	$.gritter.options = {
		position : '',
		class_name : '', // could be set to 'gritter-light' to use white
		// notifications
	...
	var Gritter = {
		...
		add : function(params) {
			...
			// Basics
			var title = params.title, text = params.text, image = params.image || '', sticky = params.sticky || false, item_class = params.class_name || $.gritter.options.class_name, icon = params.icon || $.gritter.options.icon, close_icon = params.close_icon
					|| $.gritter.options.close_icon, position = $.gritter.options.position, time_alive = params.time || '';
			 ...
			 * if(event.type == 'mouseenter'){ if(!sticky){
			 * Gritter._restoreItemIfFading($(this), number); } } else {
			 * if(!sticky){ Gritter._setFadeTimer($(this), number); } }
		 ...
		 *            remove() action on
		 * @param {Boolean}
		 *            manual_close Did we close the gritter dialog with the (X)
		 *            button
		 */
		...
		_hoverState : function(e, type) {
			...
			// Remove the border styles and hide (X) close button when you mouse
			// out
			else {
