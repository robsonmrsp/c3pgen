...
(function(factory) {
	...
	if (typeof exports == "object") {
		module.exports = factory(require("underscore"), require("backbone"), require("backgrid"), require("backbone-pageable"));
	}
	// Browser
	else if (typeof _ !== "undefined" && typeof Backbone !== "undefined" && typeof Backgrid !== "undefined") {
	...
	var PageHandle = Backgrid.Extension.PageHandle = Backbone.View.extend({
		 ...
		 *            [options.title]
		 * @param {boolean}
		 *            [options.isRewind=false]
		 * @param {boolean}
		 *            [options.isBack=false]
		 * @param {boolean}
		 *            [options.isForward=false]
		 * @param {boolean}
		 *            [options.isFastForward=false]
		 */
		initialize : function(options) {
			...
			if (this.isRewind)
				...
				pageIndex = Math.max(firstPage, currentPage - 1);
			else if (this.isForward)
				pageIndex = Math.min(lastPage, currentPage + 1);
			...
			if (((this.isRewind || this.isBack) && currentPage == firstPage) || ((this.isForward || this.isFastForward) && currentPage == lastPage)) {
				this.$el.addClass("disabled");
			} else if (!(this.isRewind || this.isBack || this.isForward || this.isFastForward) && currentPage == pageIndex) {
	...
	Backgrid.Extension.Paginator = Backbone.View.extend({
		 ...
		 *            [options.controls]
		 * @param {boolean}
		 *            [options.pageHandle=Backgrid.Extension.PageHandle]
		 * @param {boolean}
		 *            [options.goBackFirstOnSort=true]
		 */
		initialize : function(options) {
			...
			if ((options.goBackFirstOnSort || this.goBackFirstOnSort) && collection.fullCollection) {
				this.listenTo(collection.fullCollection, "sort", function() {
					collection.getFirstPage();
				});
			}
		},
		_cloneState : function(_state) {
			var state = {};

			state.firstPage = _state.firstPage;
			state.lastPage = _state.lastPage;
			state.currentPage = _state.currentPage;
			state.pageSize = _state.pageSize;
			state.totalPages = _state.totalPages;
			state.totalRecords = _state.totalRecords;
			return state;
		},
		_calculateWindow : function() {
			var collection = this.collection;
			var state = this._cloneState(collection.state);
			// robson
