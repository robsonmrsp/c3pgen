...
import ${application.rootPackage}.core.model.Owner;
...
<#else>
	// Consultas considerando o multitenancy
		public Pager<User> getAll(PaginationParams<FilterUser> queryParams) {
			FilterUser filterUser = queryParams.getFilter();
			CriteriaBuilder builder = builder();
			Root<User> root = root();
			CriteriaQuery<User> query = query();
		...
		public List<User> filter(PaginationParams<FilterUser> queryParams) {
			FilterUser filterUser = queryParams.getFilter();
		...
		public List<User> filter(PaginationParams paginationParams, Boolean equals) {
			List<User> list = new ArrayList<User>();
			FilterUser filterUser = (FilterUser) paginationParams.getFilter();

			return filter(filterUser,equals);
		}

		public List<User> filter(FilterUser filterUser, Boolean equals) {
			if (equals) {
				return filterEqual(filterUser);
			} else {
				return filterAlike(filterUser);
			}
		}

		public List<User> filterEqual(FilterUser filterUser) {
		...
		public List<User> filterAlike(FilterUser filterUser) {
			CriteriaBuilder builder = builder();
			Root<User> root = root();
			CriteriaQuery<User> query = query();
