...
public interface ${entity.name}Service {
	...
	<#if entity.hasOwner?? &&  entity.hasOwner>
	${entity.name} get(Integer id, Owner client);

	List<${entity.name}> all(Owner owner);
	
	List<${entity.name}> filter(Filter${entity.name} filter${entity.name}, Owner owner, Boolean equals);
	
	List<${entity.name}> filter(PaginationParams paginationParams, Owner owner, Boolean equals);
	
	List<${entity.name}> filter(PaginationParams paginationParams, Owner owner);
	
	Pager<${entity.name}> all(PaginationParams paginationParams, Owner owner);
	</#if>
}
//generated by JSetup ${JSetupVersion} :  at ${.now}