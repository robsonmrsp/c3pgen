...
-- registros de autorização para "${firstUpper(entity.displayName)}"	
	-- campos de tela
	INSERT INTO item(id,     description, identifier, type, name) VALUES	((select  coalesce(MAX(id), 0)  + 1 from item), 'Item de menu principal para tela de pesquisa de "${firstUpper(entity.displayName)}"', 			'item-list-"${firstUpper(entity.name)}"',	'COMPONENT', 'Menu "${firstUpper(entity.displayName)}"s'					);
	INSERT INTO persmission(id , id_item,  name, description,  operation, tag_reminder) VALUES ((select  coalesce(MAX(id), 0)  + 1  from persmission), (select  coalesce(MAX(id), 0)  from item),  'Cadastro de "${firstUpper(entity.displayName)}". Permissão de acesso ao menu de "${firstUpper(entity.displayName)}"', 'Usuário Poderá ver e acessar o item de menu de "${firstUpper(entity.displayName)}" ', 'LEITURA', 'CRUD "${firstUpper(entity.displayName)}" , item Menu ');
	...
	INSERT INTO item(id,     description, identifier, type, name) VALUES	((select  coalesce(MAX(id), 0)  + 1 from item), 'Botão de novo na listagem de "${firstUpper(entity.displayName)}"', 								'new-"${firstUpper(entity.name)}"', 		'COMPONENT', 'Botão Novo "${firstUpper(entity.displayName)}"'				);
	...
	INSERT INTO item(id,     description, identifier, type, name) VALUES	((select  coalesce(MAX(id), 0)  + 1 from item), 'Tela de edição de "${firstUpper(entity.displayName)}"', 										'app/edit"${firstUpper(entity.name)}"', 	'SCREEN', 'Edição de  "${firstUpper(entity.displayName)}"s');
	...
	INSERT INTO item(id,     description, identifier, type, name) VALUES	((select  coalesce(MAX(id), 0)  + 1 from item), 'Url de acesso aos "${firstUpper(entity.displayName)}"s por filtro', 							'crud/"${firstLower(entity.name)}"s/filter', 		'SERVICE', 			'Servico de "${firstUpper(entity.displayName)}"s sob filtro');
	...
	INSERT INTO item(id,     description, identifier, type, name) VALUES	((select  coalesce(MAX(id), 0)  + 1 from item), 'Url de acesso a um "${firstUpper(entity.displayName)}" específico', 							'crud/"${firstLower(entity.name)}"s/[0-9]', 		'SERVICE', 			'Servico para acesso a "${firstUpper(entity.displayName)}" especifico'); --GET, PUT e DELETE 
