					...
					<div class="panel panel-default plain profile-widget">
						...
						<#list entity.attributes as att>
						  <#if att.viewAproach?? >
							...
							<div id="groupInput${firstUpper(att.name)}" class="form-group">
								<label class="control-label" for="input${firstUpper(att.name)}">${firstUpper(att.displayName)!firstLower(att.name)}</label>
								<select class="form-control" data-ng-model="${firstLower(entity.name)}Filter.${firstLower(att.name)}" ng-options="${firstLower(att.name)} as ${firstLower(att.name)}.${firstLower(rel.viewAproach.comboVal)} for ${firstLower(att.name)} in ${firstLower(att.name)}s"></select>
							</div>					
						...
						<#list entity.relationships as rel>
							...
							<div id="groupInput${firstUpper(rel.name)}" class="form-group">
								...
								<select class="form-control" id="input${firstUpper(rel.name)}"  data-ng-model="${firstLower(entity.name)}Filter.${firstLower(rel.name)}" ng-options="${firstLower(rel.name)} as ${firstLower(rel.name)}.${firstLower(rel.viewAproach.comboVal)} for ${firstLower(rel.name)} in ${firstLower(rel.name)}s"></select>
							</div>					
							...
							<div id="groupInput${firstUpper(rel.name)}Container" class="form-group">
								...
								<div id="groupInput${firstUpper(rel.name)}" class="input-group">
									...
									<input id="input${firstUpper(rel.name)}${firstUpper(rel.viewAproach.hiddenField)}" type="text" class="form-control append-left" data-ng-model="${firstLower(entity.name)}Filter.${firstLower(rel.name)}.${firstLower(rel.viewAproach.hiddenField)}" style="display: none;" />
									</#if>
									<#if rel.viewAproach.textField??>							
									<input id="input${firstUpper(rel.name)}${firstUpper(rel.viewAproach.textField)}" placeholder="Pesquise pelo ${firstUpper(rel.displayName)}"  type="text" class="form-control append-left" data-ng-model="${firstLower(entity.name)}Filter.${firstLower(rel.name)}.${firstLower(rel.viewAproach.textField)}" />
									</#if>
							...
							<table class="table backgrid table-striped table-bordered table-responsive table-hover dataTable no-footer  " c3p-expose st-pipe="get${firstUpper(entity.name)}s" st-table="pager${firstUpper(entity.name)}s.itens">
								<thead>
									...
									<#if entity.relationships??>	
										...
										<th st-sort="${firstLower(rel.name)}.${rel.viewAproach.textField}">
											<a>
												<b class="direction-icon"></b>
												${rel.displayName}
											</a>
								...
								<#list entity.relationships as rel >
									<#if rel.viewAproach.type == 'modal'>
									<td>{{${firstLower(entity.name)}.${firstLower(rel.name)}.${rel.viewAproach.textField}}}</td>											
									</#if>
								</#list>																						
