...
(function(factory) {
	...
	// Browser
	else if (typeof _ !== "undefined" && typeof Backgrid !== "undefined" && typeof moment !== "undefined") {
	...
	/**
	 * MomentFormatter converts bi-directionally any datetime values in any
	 * format supported by [moment()](http://momentjs.com/docs/#/parsing/) to
	 * any datetime format
	...
	_.extend(MomentFormatter.prototype, {
		 ...
		 * @cfg {Object} options
		 * 
		 * @cfg {boolean} [options.modelInUnixOffset=false] Whether the model
		 *      values should be read/written as the number of milliseconds
		 *      since UNIX Epoch.
		 * 
		 * @cfg {boolean} [options.modelInUnixTimestamp=false] Whether the model
		 *      values should be read/written as the number of seconds since
		 *      UNIX Epoch.
		 * 
		 * @cfg {boolean} [options.modelInUTC=true] Whether the model values
		 ...
		 * 
		 * @cfg {string} [options.modelFormat=moment.defaultFormat] The format
		 *      this moment formatter should use to read/write model values.
		 *      Only meaningful if the values are strings.
		 * 
		 * @cfg {boolean} [options.displayInUnixOffset=false] Whether the
		 *      display values should be read/written as the number of
		 *      milliseconds since UNIX Epoch.
		 * 
		 * @cfg {boolean} [options.displayInUnixTimestamp=false] Whether the
		 *      display values should be read/written as the number of seconds
		 *      since UNIX Epoch.
		 * 
		 * @cfg {boolean} [options.displayInUTC=true] Whether the display values
		 *      should be read/written in UTC mode or local mode.
		 * 
		 * @cfg {string} [options.displayLang=moment.lang()] The locale the
		 *      display values should be read/written in.
		 * 
		...
		fromRaw : function(rawData) {
			//Somente para evitar o erro " Cannot call method 'utc' of null " quando a tada vem vomo string
			if (rawData == null || rawData === '')
				return '';

			var m = this.modelInUnixOffset ? moment(rawData) : this.modelInUnixTimestamp ? moment.unix(rawData) : this.modelInUTC ? moment.utc(rawData, this.modelFormat,
					this.modelLang) : moment(rawData, this.modelFormat, this.modelLang);

			if (this.displayInUnixOffset)
				return +m;

			if (this.displayInUnixTimestamp)
				return m.unix();
		...
		toRaw : function(formattedData) {
			...
			var m = this.displayInUnixOffset ? moment(+formattedData) : this.displayInUnixTimestamp ? moment.unix(+formattedData) : this.displayInUTC ? moment.utc(formattedData,
					this.displayFormat, this.displayLang) : moment(formattedData, this.displayFormat, this.displayLang);
	...
	var MomentCell = Backgrid.Extension.MomentCell = Backgrid.Cell.extend({
		 ...
		 *           [formatter=Backgrid.Extension.MomentFormatter]
		 */
